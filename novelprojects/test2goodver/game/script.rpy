...
label start:
    ...
    selen "Тайна, которую я хотела тебе рассказать..."
    you "..."
    selen "..."
    selen "Знаешь почему ты здесь?"
    you "..."

    $ renpy.music.set_volume(0.9, delay=1.0)
    show stars at Transform(alpha=0.3, pos=(0.27, 0.08), zoom=1.6) with dissolve

    selen "Одиночество."
    selen "Ты сам запер себя в этом мире."
    selen "Ты боялся быть один, ты не мог этого вынести."
    selen "Ты создал меня, лишь бы скрыться от всего этого."
    selen "Что ты выберешь..."
    selen "Принять реальность или дальше скрываться в иллюзии?"

    # Убираем меню выбора и сразу запускаем мини-игру
    you "Я... Я не знаю."
    selen "Докажи свою силу воли. Переживи испытание."
    selen "Если выдержишь - найдешь путь к реальности."
    selen "Если падёшь - останешься здесь навсегда."

    # Останавливаем текущую музыку
    stop music fadeout 2.0

    # Переход к мини-игре
    you "Что это за испытание?"
    ...
    selen "Битва с твоими страхами."


    # Текст перед битвой
    scene black
    show expression Text("ПРИГОТОВЬСЯ", size=72, color="#FF0000", bold=True) as text1:
        alpha 0.0
    ...
    show expression Text("СМЕЛОСТЬ БУДЕТ ИСПЫТАНА", size=48, color="#FF0000", bold=True) as text2:
        alpha 0.0
